...
after successful create / `--cd` / `--dep` / `--bring` / `--set-task` / `--done`), and
...
- **wrk --dep** — spawns a dependency worktree under `{consumerTop}/external/` as a worktree of the DEP repo (`git -C <depMain> worktree add`), so it is registered under `<depMain>/.git/worktrees/` (NOT the consumer's); the dep already holds its own objects, so no remote/fetch into the consumer is needed. **Consumer module discovery**: scans the full consumer tree (`gotool/mod/scan.Scan`) for all Go modules — consumers without a root `go.mod` (module lives in a subdirectory like `go-pkgs/`) are supported. **Dep module discovery**: likewise scans the dep tree for all Go modules. Matches each dep module against every consumer module's `require`/`replace` directives; for every matching consumer module, runs `gotool.Replace` + `gotool.Tidy`. Appends `/external` to `.gitignore` when missing, prints external worktree abs path on stdout. Path naming: `{basename}-{token}-{WRK_DATE}[-N]` (basename from dep main repo). Branch naming: `{token}-{WRK_DATE}[-N]` — **no** dep basename on the branch (aligned with create). Always `worktree add -b` (no branch reuse / no `--no-checkout` on create). Collision is joint path+branch against `depMain`. Hard-errors when dep is not a go module, consumer has no modules, or dep is not required/replaced by any consumer module. **Same-repo reuse (Policy A)**: before creating, resolve `depMain := ResolveMainRepo(depPath)` and find live linked worktrees of `depMain` under `{consumerTop}/external/`; if any, **reuse** the lexicographically smallest abs path (no new worktree/branch); still ensure `/external` gitignore and apply replace/tidy as today. Multiple matches → reuse smallest; warn count + list extras on **stderr** (`already has <N> worktrees under external/; reusing …`, `also present: …`). Single match warning: `already exists under external/; reusing <absPath>`. stdout remains path-only. No `--force` override.
- **wrk --bring** — like `--dep` for external worktree materialization (same path/branch naming under `{consumerTop}/external/`, same dep-owned worktree, same `/external` gitignore, same abs path on stdout), but Go module analyse/replace is **best-effort**. Always creates the external worktree + gitignore when path/git resolve and worktree create succeed. Soft SKIP (exit 0 + worktree + gitignore + stdout path + stderr notice) when: dep is not a Go module; consumer has no Go modules; or modules exist but dep is not required/replaced by any consumer module. SKIP stderr substrings: `SKIP local dep replacement: <depPath> is not a go module`; `SKIP local dep replacement: consumer has no Go modules`; `SKIP local dep replacement: <depPath> is not a dependency of any consumer module`. On match → `replace` + tidy like `--dep`. Flag name `--bring` only (no alias). Mutually exclusive with `--dep` / `--all-deps` and the same exclusive mode set as `--dep`. Basename fallback same as `--dep` (`DepMode`). Event `command: "bring"`. `--exec` after successful `--bring` (including SKIP) runs in the external worktree. **Same-repo reuse (Policy A)**: identical external scope and lex-smallest reuse rules as `--dep` (search only under `{consumerTop}/external/`; stderr reuse warnings; stdout path only).
...
- **Process-local fixtures** — inject via `d *session.Doctest`. Seeds are process-local under an in-memory mutex (macOS `cp -c`, Linux `cp -a` CoW into leaf workdirs); `getWrkBin` builds wrk once per process under a mutex.
...
- **wrk --scan-git-repos [ROOT...]** — standalone mode; mutually exclusive with other modes (`--projects`, `--add`, `--list`, create, etc.). Discovers git directories under each root via `scan_repo.Scan` (default product CacheRoot unless overridden), filters **`RepoTypeMain` only** (skips linked worktrees), and records each main with `storage.RecordProject(wrkHome, path, source)` where source is **`scan`** (`storage.SourceScan = "scan"`). **Roots**: remaining positionals after flags; when no roots are given, default root is **`$HOME` (`~`)** if it is a directory (not `~/Projects`); empty/unresolvable/non-directory/side-effect-only home → clear error mentioning home or `~`. **Two-base cache + path filter (P5)**: each CLI root under `$HOME` (including HOME itself) maps to universe **`home`** and shares product cache files under `$HOME/.cache/git-repo-scan/home/` (`repos.json` durable index, `walk.jsonl` + `walk.cursor.json`, `meta.json`); roots **outside** home map to universe **`root`** (`…/root/…`). Scan still passes the user-provided roots so discovery walks those paths, while the home-universe index can retain siblings outside a subpath root (e.g. bare `~` then later `~/Projects` reuses the same `home/repos.json`; emit/record is **filtered** to paths under the CLI roots only — scanning `~/Projects` does not print/record `~/other-main`). Empty `CacheRoot` uses product default `$HOME/.cache/git-repo-scan` (library also maintains per-dir `mirror/` entries and adaptive walk-log consume budgets — see `scan_repo` tests). **Streaming / OnRepo**: for each newly discovered main under the filter, **record-as-found** then print immediately via `scan_repo.Options.OnRepo` — **discovery order** (CLI root order + walk order), **not** sorted after the full scan. **Stdout**: each **newly** recorded absolute main path on its own line as soon as it is found (trailing `\n` after last); already-known paths silent on stdout (optional summary on stderr). Exit 0 on success; partial root errors may warn on stderr. **Ctrl-C / SIGINT / SIGTERM**: cancel the scan context, keep partial `projects.json` (and scan cache already written), write stderr `warning: scan interrupted; progress saved (cache and newly recorded projects)` (must include `warning:` plus interrupt/progress-saved intent), and exit **130** via silent `ExitCodeError{Code: 130}` (no second hard-error body). **`--no-cache`**: bool flag valid **only** with `--scan-git-repos`; passes `NoCache: true` to `scan_repo.Scan` (no cache read/write); bare `wrk --no-cache` → non-zero, stderr `wrk: --no-cache is only valid with --scan-git-repos`. **Debug**: `scan_repo.Options.Debug = verbose || envTruthy(WRK_SCAN_DEBUG)` where truthy is `1`/`true`/`yes` (case-insensitive); pass `Options.Stderr = os.Stderr`. When Debug is on, stderr includes greppable phase-level `scan:` lines (`mode=warm|cold`, cache root, serve/refresh timing) plus wrk-side **two-base** lines `scan: cache_base=home|root filter=<abs-root>` per CLI root and optional summary `scan: record known=N newly=M`; when off, zero `scan:` markers from the scan path. Help documents `--scan-git-repos`, `--no-cache`, and default root `~`. Event `command: "scan-git-repos"`.
...
│       └── second-dep/           # second --dep → same path + replace; reuse warning; no -1
├── bring/                        # wrk --bring best-effort local dep replacement
...
│   ├── mutual-exclusive-with-dep/ # --bring + --dep → non-zero mutually exclusive
...
│   │   ├── multi-dep-branch-names/ # two deps on main → branch main-{date} each (no basename)
...
│   │   ├── reuse-existing-external/ # prior external WT of depMain → reuse + replace; no second WT
...
│   │       └── reuse-existing-external/ # would: uses existing name; reuse warn; no writes
...
| 20 | dep/basic | Consumer requires dep; `--dep` creates external wt; branch `main-{date}` (no basename); replace, tidy, gitignore |
| 20a | dep/branch-collision-suffix | Prefers `main-{date}` taken in dep → path+branch `-1`; no dep basename on branch |
...
| 34e | bring/mutual-exclusive-with-dep | `--bring` + `--dep` → non-zero, mutually exclusive |
...
| 34j | dep/reuse-same-repo/second-dep | second `--dep` same dep → same path + replace; reuse warning; no `-1` |
| 35 | all-deps/registered/basic | dep1+dep2 registered in projects.json → both linked in project-path order, `wrked 2 deps` |
| 35a | all-deps/registered/multi-dep-branch-names | two deps both on main → branch `main-{date}` each (no dep basename) |
...
| 47 | all-deps/dry-run/without-all-deps | bare `wrk --dry-run` → non-zero; stderr host list: done, merge-back, all-deps, tag-next, propagate-tags, sync |
...
| 47d | all-deps/registered/reuse-existing-external | prior external WT of depMain (replace dropped) → reuse path + re-apply replace; no second WT |
| 47e | all-deps/registered/dry-run/reuse-existing-external | dry-run with existing external → `would:` uses existing name; reuse warn; no replace write |
...
| 214 | scan-git-repos/streaming/discovery-order | multi-root CLI order on stdout (not lex path sort); both `source=scan` |
| 215 | scan-git-repos/streaming/first-path-before-finish | `label:slow`; first newly recorded path bytes before process exit (pad walk) |
| 216 | scan-git-repos/interrupt/sigint-after-first-path | SIGINT after first path → exit 130; `warning:` progress saved; partial projects |
...
| 223 | scan-git-repos/filter-home-subpath/projects-shares-home-cache | `~/Projects` reuses home universe cache; stdout/projects only under Projects |
...
doctest test ./tests/dep/branch-collision-suffix
doctest test ./tests/all-deps/registered/multi-dep-branch-names
...
doctest vet ./tests/dep/reuse-same-repo
doctest test ./tests/dep/reuse-same-repo
doctest test ./tests/all-deps/registered/reuse-existing-external
doctest test ./tests/all-deps/registered/dry-run/reuse-existing-external
