...
│   └── main-flag/                # wrk --main --status: status of main repo (no shell)
│       ├── happy/                # successful composition; stdout == --status from main
│       │   ├── from-external-wt/ # cwd = external wrk worktree
│       │   │   ├── main-then-status/   # Args --main --status
│       │   │   └── status-then-main/   # Args --status --main
│       │   ├── already-at-main/  # cwd = main root
...
| 88z1 | status/main-flag/happy/from-external-wt/main-then-status | `--main --status` from external wt == `--status` from main |
| 88z2 | status/main-flag/happy/from-external-wt/status-then-main | `--status --main` same stdout as main-then-status / main status |
| 88z3 | status/main-flag/happy/already-at-main | `--main --status` at main root == plain `--status` |
| 88z4 | status/main-flag/happy/from-in-tree-linked | full main status; not linked-cwd shortcut |
