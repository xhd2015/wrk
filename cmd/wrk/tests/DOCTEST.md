...
- **wrk --done / --merge-back post-pipeline (sync → tag-next → push)** — after successful primary (not aborted / not dry-run apply), optional post steps run in **fixed order** regardless of flag argv order: **sync → tag-next → push** (then **exec → land** on the **done** path only; merge-back keeps the source worktree and has no exec/land). Subsets allowed (e.g. `--tag-next` only, `--sync --tag-next`, `--tag-next --push`, `--push` only, full combo). **Blank line** between major stdout stages. When `--tag-next` is set with primary `--done` or `--merge-back`, dispatch must be the **primary** (not bare `runTagNext`); tag-next apply runs on **main** after merge (root-bump style: plan/apply human lines + `N tag created`). Abort of primary: no post steps. Hard fail of a post step: stop, no undo. Event `command` stays `"done"` / `"merge-back"` (args may include `--sync` / `--tag-next` / `--push`). Coverage: flag matrix `done-compose/`; branch push `done-push/`; real apply `done-pipeline/` + `merge-back-pipeline/`; composition dry-run under each tree’s `dry-run/`.
...
- **--dry-run** — bool flag (no value); valid with **`--all-deps`**, **`--tag-next`**, **`--sync`**, and **primary composition** (`--done` / `--merge-back`, alone or with post modifiers `--sync` / `--tag-next` / `--push`). Bare `wrk --dry-run` (no host) → non-zero exit, stderr `wrk: --dry-run is only valid with --done, --merge-back, --all-deps, --tag-next, or --sync`. With primary + optional post flags: plan **all requested stages** (primary merge-back plan, cascade `would:` lines when applicable, then would-be sync / tag-next plan / push) with **zero mutations** and **no confirm prompts** (no `-y` required) — see `done-pipeline/dry-run/` and `merge-back-pipeline/dry-run/`. Standalone `--all-deps --dry-run` and `--tag-next --dry-run` unchanged. It does NOT relax `--all-deps`'s mutual exclusion with `--dep`/`--done`/`--list` — `--dry-run --all-deps --dep <x>` still errors as mutually exclusive (the `--all-deps` mutual-exclusion check runs first).
...
├── done-pipeline/                # done post-pipeline sync → tag-next → push (real apply)
...
│   ├── aborted-full-flags/       # decline confirm → no sync/tag/push
...
├── merge-back-pipeline/          # merge-back post-pipeline sync → tag-next → push (real apply; wt kept)
...
├── done-compose/                 # flag matrix: primary + post modifiers (tag-next/push/sync/dry-run)
...
| 47 | all-deps/dry-run/without-all-deps | bare `wrk --dry-run` → non-zero; stderr still contains `--dry-run is only valid with --all-deps` (full host list also names done/merge-back/tag-next/sync) |
...
| 68w | done-pipeline/aborted-full-flags | decline with full flags → no sync/tag/push; wt remains |
