...
- **wrk --bring** — sole external-dep worktree mode. Spawns a dependency worktree under `{consumerTop}/external/` as a worktree of the DEP repo (`git -C <depMain> worktree add`), registered under `<depMain>/.git/worktrees/` (NOT the consumer's). **Consumer module discovery**: scans the full consumer tree (`gotool/mod/scan.Scan`) for all Go modules — consumers without a root `go.mod` (module in a subdirectory) are supported. **Dep module discovery**: scans the dep tree for all Go modules. Matches each dep module against every consumer module's `require`/`replace`; on match runs `gotool.Replace` + `gotool.Tidy`. Appends `/external` to `.gitignore` when missing; prints external worktree abs path on stdout. Path naming: `{basename}[-N]`. Branch naming: `{token}-{WRK_DATE}[-N]` (no dep basename; always `worktree add -b`). Path and branch `-N` are independent: path `-N` only when the preferred external name is occupied (foreign worktree / plain dir / other); a taken preferred branch walks branch `-N` alone and does not bump the path (stderr `warning: branch … exists; using …-N`). **Best-effort module analyse**: soft SKIP (exit 0 + worktree + gitignore + stdout path + stderr notice) when: dep is not a Go module; consumer has no Go modules; or modules exist but dep is not required/replaced by any consumer module. SKIP stderr substrings: `SKIP local dep replacement: <depPath> is not a go module`; `SKIP local dep replacement: consumer has no Go modules`; `SKIP local dep replacement: <depPath> is not a dependency of any consumer module`. Non-git consumer cwd: materialize under `{abs(cwd)}/external/`; soft-skip replace + gitignore. Flag name `--bring` only (no `--dep` alias). **`Varargs("--bring", &bringPaths)`**: consume ≥1 non-flag token per occurrence, stop at the next token starting with `-`, continue parsing; repeat appends. Bare `--bring` / `--bring --no-dep` → requires a value (library wording). **Exclusive bring** (`wrk --bring x1 x2`, no create form) is mutually exclusive with `--done` / `--list` / `--merge-back` / `--set-task` / `--cd` / `--main` / pipeline stages; consumer is **cwd**; event `command: "bring"`; `args` include `--bring` and every path; `--exec` after exclusive `--bring` (including SKIP) runs in the external worktree (exclusive multi-bring + `--exec` still rejected). **Create compose** (`wrk src --bring …`, `wrk src dst --bring …`, `wrk src -t … --bring …`, `wrk --new --bring …` and mixes): `--task` / `--new` / `<target-dir>` / create UX flags are **not** exclusive with `--bring`; `preflightResolveBringArgs` runs before window/create; bring applies inside the **new** (or Policy B reused) worktree; event `command: "create"` (not `"bring"`); `args` include `--bring` and every path (and `-t`/`--new` when passed); `--exec` `cmd.Dir` is the **project** worktree. `wrk --bring x1 x2 src` treats `src` as a dep path, not a project. Basename fallback via same `resolveDirArg` core as create (`allowBasenameFallback=true`). **Same-repo reuse (Policy A)**: live linked worktrees of `depMain` under `{consumerTop}/external/` → reuse lex-smallest (stderr reuse warnings; stdout path only). **`--no-dep`**: long-only bool valid **only** with `--bring`; materialize/reuse external worktree + gitignore; **skip module match scan entirely** (no SKIP messages; no replace/tidy); go.mod unchanged. Bare `--no-dep` or with other modes → non-zero, stderr `wrk: --no-dep is only valid with --bring`. With `-v`, may log/stream git worktree add but never logs `go mod tidy` under `--no-dep`.
- **wrk --install name...** — exclusive named install of local module binaries: resolution is shared with `--reinstall-local` (`cmd/<name>` → `go install`; `script/<name>/install` → `go run`; nearest-`go.mod` re-root; same `notice:`/`warning:` diagnostics), but every selected item is **forced** (`Action=install`), so there is **no GOBIN/GOPATH/bin presence gate** — the name installs even when it was never installed, and `skip:` lines never appear. `Varargs("--install", …, WithMinimum(1))`: names are arbitrary non-flag tokens like `--bring p1 p2`; bare `--install` (or `--install` followed only by flags) → library wording `--install requires a value`. Partners: `--main` (scan main repo modules), `--dry-run`, `--color`/`--no-color`, `-v`; exclusive with all other modes (`wrk: --install is mutually exclusive with other modes`) and explicitly with `--reinstall-local` (never a pipeline/compose stage). Dry-run stdout: `would: go install <ownRel>` / `would: go run <ownRel>`, then `would: install N binaries` (K=1) or `# module …` headers + `would: install N binaries across K modules` (K>1). Execute stdout: `go install <ownRel>` progress lines then `installed N, failed F`; failures stay **soft** (exit 0 + stderr `warning: install finished with F failed`). Hard errors (non-zero, prefixed `wrk: --install:`) for unknown (`no install candidate`), ambiguous (`bin "<name>" is ambiguous (…)`), or multi-module-claimed (`claimed by multiple modules`) names. Event `command: "install"`; `args` include `--install` and each name (plus `--main` / `--dry-run` when passed). Coverage: `install-cli/` (L2 in-process default; `execute/installs-bin` is L3 e2e).
...
- **wrk --push (dual meaning)** — never bare (`wrk --push` alone → non-zero). **(1) Branch under primary**: with `--done` or `--merge-back` and **without** `--tag-next`, after successful primary (not aborted / not dry-run apply), call `runPushMain` from **main** with empty tag refs — pushes the main branch (flag order free: `--done --push` ≡ `--push --done`; same for merge-back). When `origin/<worktree-branch>` already exists, its pre-land tip is snapshotted before MergeBack; after main is pushed, if that remote tip is unchanged **and** was already included in the local branch, also `git push --force-with-lease` the post-land tip onto `origin/<worktree-branch>` (stdout second line `pushed <branch> → origin/<branch>`). Tip moved / not included / push failure → `warning:` on stderr, exit 0 (PR-branch update is optional). Missing same-name remote ref → no extra push (do not create). **(2) Tags with standalone tag-next**: `wrk --tag-next --push` pushes newly created tag refs only (no branch push). **(3) Both when primary + tag-next + push**: create tags locally first, then `runPushMain(main, false, createdTags)` pushes **branch + tags** (same optional same-name origin update as (1)). **Remote resolution** (primary path): prefer configured upstream of main’s current branch; else fallback `origin` + current branch name. **Stdout** (primary path): blank line before push stage, then stable confirmation `pushed main → origin/main\n` (plus same-name line when updated). Push hard-fail of **main** after primary success → non-zero exit; **no undo** of primary. No resolvable remote → clear non-zero error.
- **wrk --done / --merge-back post-pipeline (sync → tag-next → push)** — after successful primary (not aborted / not dry-run apply), optional post steps run in **fixed order** regardless of flag argv order: **sync → tag-next → push** (then **reinstall-local → exec → land** on the **done** path only; merge-back keeps the source worktree and has no exec/land). Subsets allowed (e.g. `--tag-next` only, `--sync --tag-next`, full combo). **Blank line** between major stdout stages. When `--tag-next` is set with primary `--done` or `--merge-back`, dispatch must be the **primary** (not bare `runTagNext`); tag-next apply runs on **main** after merge. Abort of primary: no post steps. Hard fail of a post step: stop, no undo. Event `command` stays `"done"` / `"merge-back"` (args may include `--sync` / `--tag-next` / `--push`). Coverage: flag matrix `done-compose/`; branch push `done-push/`; real apply `done-pipeline/` + `merge-back-pipeline/`; composition dry-run under each tree’s `dry-run/`. Stack pin/ship → `--unwind`; intentional require bumps → `--dep-update` (not a compose post stage).
...
- **--no-in-module-replace** — bool flag (no value); valid ONLY with `--done`. Restores the fully-strict local-replace guard: every filesystem/local `replace` (intra-repo or extra-repo) blocks `--done`. Without it (default), relative intra-repo replaces — whose target dir exists and shares the consumer's `git rev-parse --show-toplevel` (`../../`/`./sub` nested-module reference) — are silent and `--done` proceeds; absolute intra-repo replaces still WARN; extra-repo replaces (`./external/foo` dep worktree, non-existent target, absolute path to another checkout, sibling) still block. Bare `wrk --no-in-module-replace`, or with any other mode (`--bring`/`--list`/no-args create) → non-zero exit, stderr `wrk: --no-in-module-replace is only valid with --done`.
...
- **external/** — dependency worktrees live at `{consumerTop}/external/{basename}[-N]`; not under `WRK_HOME`. They are linked worktrees of the DEP repo (registered under `<depMain>/.git/worktrees/`), not the consumer — the consumer only hosts the working tree on disk.
...
- **Request.InProcess** — when true, `Run` uses `wrkcli.Capture` (L2) with isolated `WRK_HOME`/`WRK_DATE` overrides (no product binary). Short paths only; leave false for L3 e2e. Dual-mode on mega-tree root and nested roots: `sync/`, `gen-commit-msg/`, `projects-dep-graph/`, `install-cli/`, `reinstall-local-cli/`, `bash-integration/`, `bash-integration-install-messaging/`, `followup-cd/`, `tag-next/`, `status/main-repo-worktrees/`.
- **Request.StdinInput** — when non-empty, piped to wrk stdin before wait (mvd merge-back pattern).
- **--dry-run** — bool flag (no value); valid with **`--tag-next`**, **`--sync`**, **`--push`**, **`--install`**, **`--reinstall-local`**, **`--unwind`**, **`--pin-locals`**, **`--dep-replace`**, **`--dep-update`**, **`--gen-commit-msg`**, manual **`--commit -m`**, and **primary composition** (`--done` / `--merge-back`, alone or with post modifiers `--sync` / `--tag-next` / `--push` / `--reinstall-local`). Bare `wrk --dry-run` (no host) → non-zero exit, stderr lists valid hosts (excludes removed `--all-deps`). With primary + optional post flags: plan **all requested stages** (primary merge-back plan, cascade `would:` lines when applicable, then would-be sync / tag-next plan / push / reinstall) with **zero mutations** and **no confirm prompts** (no `-y` required) — see `done-pipeline/dry-run/` and `merge-back-pipeline/dry-run/`. Standalone `--tag-next --dry-run` unchanged.
- **wrk --task <desc>** / **wrk -t <desc>** — `-t` and `--task` are equivalent (like `-l,--list`); `hasArg` / `taskFlagSet` detect both forms. Event `args` record whichever form was passed (e.g. `["-t", "desc"]` when `-t` is used). Flag valid only in create mode (no `--done`/`--list`). Compose with `--bring` is allowed (bring applies inside the new worktree). Derives a sanitized slug from `<desc>` (lowercase, non-letter-digit → `-`, collapse, trim, truncate 64 runes soft cap). Appends slug after the date in both dir and branch names: `{basename}-{token}-{date}-{slug}[-N]` for dir, `{token}-{date}-{slug}[-N]` for branch (token = sanitize(currentBranch); no `/`). Empty `<desc>` or slug → non-zero exit. No metadata file stored — the slug is embedded in the name. **Name budget**: after soft cap, further shorten slug so path last component and branch stay ≤ **255 bytes** (reserve **3** for `-99` suffix → fit target 252); never silently chop basename/token — if prefix alone exceeds budget → clear non-zero error. Wrk-managed invariant remains `filepath.Base(path) == basename + "-" + branch`. Agent `${task}` / create-UX prompt always receives the **full original** task text (`taskDesc`), never the fitted slug alone.
...
- **create UX (window / terminal / agent)** — first-class create-mode UX from `$WRK_HOME/config.json` `create` section + one-shot CLI flags, implemented in-process via `computer-use/macos/space` (Mission Control Desktop) and `shell/iterm2` (iTerm2 open + follow-ups). Schema: `create.window.mode` (`"new"` only in v1; absent = window off), `create.terminal.mode` (`"new"` | `"reuse"` | `"smart"`; absent = terminal off), `create.agent` (`enabled` bool, `runner` default `grok-tty`, `prompt_template` default `/brainstorm ${task}` for grok/grok-tty or `$brainstorm ${task}` for codex/codex-tty when not explicitly set, `args` default `["--session-id-from-prompt","--no-submit","--open","--color"]`). Long agent prompts (and follow-ups that would exceed iTerm write-text SafeMax) are delivered as `agent-run run … --prompt-file=<abs>` via `agentrunapi.MaybeSpillPrompt` (not an inline positional prompt). Legacy `create.interceptor` (and interceptor-only keys) are **silently ignored**; interceptor code/flags/env (`--interceptor`, `--no-interceptor`, `WRK_NO_INTERCEPTOR`, template argv engine) are **removed**.
- **create UX flags (create mode)** — `--new-window` (window on + implies terminal `new` unless another terminal mode flag sets reuse/smart), `--new-terminal` / `--reuse-terminal` / `--smart-terminal` (mutually exclusive), `--open-in-agent` / `--no-open-in-agent`, `--no-new-window`, `--no-new-terminal`, `--here` (alias of no-window/no-terminal; with agent prefers follow-up `cd`+`agent-run`, else nested shell). Conflicts: open/no-open together, new-window/no-new-window, terminal-on/no-new-terminal, **`--new-window` + `--no-new-terminal`**, multiple terminal mode flags. Effective merge for plain create (`wrk [dir]`, no second positional): load config create.* (ignore interceptor) → apply CLI → if window on && terminal off → terminal=`new` → validate. **Create-with-target-dir** (`wrk [dir] <target-dir>`, `spawnTarget` non-empty): **skip** config create.* entirely (silent; not an error); empty base + CLI flags only → same window-implies-terminal-new after flags → validate. One-shot UX flags remain valid with `<target-dir>`. Pipeline after native create + path stdout: window (`space.CreateAndActivate`) → `--here`+agent (follow-up `cd`+`agent-run`, else nested shell) **or** terminal (`iterm2.OpenConfig` with optional agent follow-up) **or** agent-in-current-process (`agent-run run <args> --agent-runner=<runner> <prompt>`) → `--exec` → follow-up cd. Never double-run agent (terminal+agent ⇒ iterm follow-up only; `--here`+agent ⇒ follow-up/nested only). Non-darwin window/terminal → clear platform error.
...
├── create-bring/                 # create + --bring compose (bring into new/reused WT)
│   ├── success/
│   │   ├── from-src/             # wrk src --bring …
│   │   │   ├── two-deps/         # wrk src --bring d1 d2
│   │   │   └── with-task/        # wrk src -t 'fix login' --bring d1
│   │   ├── from-new/             # wrk --new --bring … from src cwd
│   │   │   ├── one-dep/
│   │   │   ├── no-dep/
│   │   │   └── with-exec/        # --exec pwd in project WT
│   │   └── target-dir/           # wrk src dst --bring (relocated from with-other-mode)
│   │       └── spawn-exact/
│   ├── fail/
│   │   ├── preflight-unknown/    # no-such-basename → no new WT
│   │   └── apply-second-not-git/ # keep WT; no rollback
│   └── still-exclusive/
│       └── with-done/            # wrk --done --bring stays exclusive
├── bring/                        # wrk --bring sole external-dep worktree mode (best-effort replace)
│   ├── basic/                    # require + --bring → external wt + replace (no SKIP)
│   ├── branch-collision-suffix/  # preferred branch taken → path stays; branch -1 + warning
...
│   ├── multi/                    # Varargs --bring p1 p2 + repeat --bring
│   │   ├── two-success/          # --bring p1 --bring p2 (repeat still works)
│   │   ├── varargs-two/          # --bring p1 p2 (one flag, two values)
│   │   ├── varargs-no-dep/       # --bring p1 p2 --no-dep (stops at flag)
...
│   │   └── reject/               # exec / missing-value / exact duplicates
...
│   ├── intra-replace-warns/      # relative intra-repo fs replace (./submod, same toplevel) → silent proceed (default, exit 0)
│   ├── intra-replace-abs-warns/  # absolute intra-repo fs replace (same toplevel) → WARN + proceed (default, exit 0)
...
├── merge-back-push-pr-branch/    # land --push also lease-updates existing origin/<WtBranch>
│   ├── merge-back-ahead/         # FF land; origin/<B> == main HEAD
│   ├── merge-back-diverged/      # rebase land; origin/<B> rewritten to new tip
│   ├── done-ahead/               # --done still updates origin/<B> after branch -D
│   ├── done-diverged/            # --done rebase + origin/<B> lease-update
│   ├── not-included/             # remote-only tip → warning; origin/<B> unchanged
│   └── dry-run/                  # would: force-with-lease; no remote mutation
...
├── done-pipeline/                # done post-pipeline sync → tag-next → push → reinstall-local
...
│   ├── aborted-full-flags/       # decline confirm → no sync/tag/push/reinstall
...
├── merge-back-pipeline/          # merge-back post-pipeline sync → tag-next → push
...
│   │   ├── untracked-dirty/      # ?? untracked file → dirty (… 1 untracked)
│   │   └── nested-untracked-dirty/ # primary + header + nested dirty (1 staged)
│   ├── invalid-git-cwd/
│   │   └── non-git/              # cwd is not a git repo (error)
...
│   ├── with-other-mode/          # target-dir + non-create mode → error
│   │   └── with-list/            # wrk <dir> <target-dir> --list (--bring allowed; see create-bring/)
│   └── reuse-same-repo/          # Policy B: scoped same-parent reusable sibling reuse
...
│           └── reusable/         # clean + HEAD==source
│               ├── tty/          # UseScriptTTY + stdin Y/n
│               │   ├── skip-default/ # Enter → skip; stdout = sibling
│               │   ├── proceed-n/    # n → create under target
...
| 60 | create-bring/success/target-dir/spawn-exact | `wrk src dst --bring d1` → create at `dst` + bring into that path (relocated) |
| 60a | target-dir/reuse-same-repo/no-prior-linked | named create, no prior linked WT → create under target; no Policy B banner |
| 60b | target-dir/reuse-same-repo/exact-path-exists | intended spawn path already exists → non-zero error |
| 60c | target-dir/reuse-same-repo/sibling-other-parent | WT only under other parent (WRK_HOME) → create; no banner |
| 60d | target-dir/reuse-same-repo/sibling-same-parent/not-reusable/dirty | dirty same-parent sibling → create; no banner |
| 60e | target-dir/reuse-same-repo/sibling-same-parent/not-reusable/clean-differs-from-source | clean HEAD differs → create; no banner |
| 60f | target-dir/reuse-same-repo/sibling-same-parent/reusable/tty/skip-default | TTY + Enter → skip; stdout = sibling (label: tty) |
...
| 66 | done/intra-replace-warns | relative intra-repo `replace example.com/foo => ./submod` (existing, same toplevel) → silent, exit 0, merge-back proceeds |
| 66a | done/intra-replace-abs-warns | absolute intra-repo replace to same-toplevel `submod` → WARN + proceed, exit 0 |
...
| 68r1 | merge-back-push-pr-branch/merge-back-ahead | `--merge-back -y --push` also lease-updates existing origin/<WtBranch> |
| 68r2 | merge-back-push-pr-branch/merge-back-diverged | rebase land rewrites origin/<WtBranch> to new tip |
| 68r3 | merge-back-push-pr-branch/done-ahead | `--done -y --push` updates origin/<WtBranch> after branch delete |
| 68r4 | merge-back-push-pr-branch/done-diverged | `--done` rebase + origin/<WtBranch> lease-update |
| 68r5 | merge-back-push-pr-branch/not-included | remote-only tip → warning; origin/<WtBranch> unchanged; exit 0 |
| 68r6 | merge-back-push-pr-branch/dry-run | dry-run plans force-with-lease; origin refs unchanged |
...
| 68w | done-pipeline/aborted-full-flags | decline with full flags → no post stages; wt remains |
...
| 86a | status/valid-git-cwd/untracked-dirty | clean repo + one untracked `??` file → dirty `(0 staged, 0 changed, 0 renamed, 0 deleted, 1 untracked)` |
| 86b | status/valid-git-cwd/nested-untracked-dirty | root clean; nested `tools/child` with only untracked → child dirty `(1 staged, …)` |
...
| M6b | bash-integration/complete/bring-second-value | second `--bring` value still completes (`wrk --bring already-typed be` → `beta`) |
| M7 | create-bring/success/target-dir/spawn-exact | target-dir + `--bring` allowed (relocated from `with-other-mode/with-bring`) |
| V1 | bring/multi/varargs-two | `wrk --bring p1 p2` same side effects as repeat form (relocated from `reject/positional`) |
| V2 | bring/multi/varargs-no-dep | `wrk --bring p1 p2 --no-dep` stops at flag; no replace |
| V3 | bring/multi/reject/missing-value/bare | `wrk --bring` → requires a value |
| V4 | bring/multi/reject/missing-value/next-is-flag | `wrk --bring --no-dep` → requires a value |
| CB1 | create-bring/success/from-src/two-deps | `wrk src --bring d1 d2` → new WT; bring into that WT; event create |
| CB2 | create-bring/success/from-src/with-task | `wrk src -t 'fix login' --bring d1` → slug + bring |
| CB3 | create-bring/success/from-new/one-dep | `wrk --new --bring d1` from src; event create |
| CB4 | create-bring/success/from-new/no-dep | `--new --bring d1 --no-dep` → WT + external; no replace |
| CB5 | create-bring/success/from-new/with-exec | `--new --bring d1 --exec pwd` → pwd in **project** WT |
| CB6 | create-bring/fail/preflight-unknown | `--new --bring no-such-basename` → no new WT |
| CB7 | create-bring/fail/apply-second-not-git | first ok, second not git → keep WT; no rollback |
| CB8 | create-bring/still-exclusive/with-done | `wrk --done --bring d1` stays mutually exclusive |
...
# Varargs --bring + create compose (expect RED until Varargs + compose land)
doctest vet ./tests/create-bring ./tests/bring/multi/varargs-two ./tests/bring/multi/varargs-no-dep ./tests/bring/multi/reject/missing-value
doctest test ./tests/create-bring
doctest test ./tests/bring/multi/varargs-two
doctest test ./tests/bring/multi/varargs-no-dep
doctest test ./tests/bring/multi/reject/missing-value
...
doctest test ./tests/done/intra-replace-abs-warns
