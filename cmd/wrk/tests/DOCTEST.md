...
│   └── no-dep/                   # --dep --no-dep: worktree only (strict pre-checks)
│       ├── worktree-no-replace/  # matching dep → wt + gitignore; go.mod unchanged
│       └── still-strict-not-a-dependency/ # not required → error; no external wt
...
│   │   ├── skips-replace-tidy/   # matching dep → wt; go.mod byte-identical
│   │   ├── with-verbose-no-tidy-log/ # -v may log git; never mod tidy
│   │   ├── not-a-dep-still-works/ # no require → wt; no SKIP (analyse skipped)
│   │   └── invalid/              # --no-dep without host mode
│   │       ├── bare/             # wrk --no-dep → only-valid-with message
│   │       └── with-list/        # --list --no-dep → only-valid-with message
│   ├── verbose/                  # -v with --bring: tidy logs + worktree stream
│   │   ├── tidy-pre-line/        # -v logs $ go -C … mod tidy
│   │   ├── no-verbose-silent-tidy/ # without -v: no tidy pre-line
│   │   └── streams-worktree-add/ # -v streams Preparing worktree
...
│   │   ├── no-dep/             # --all-deps --no-dep → wts; no replaces; wrked N
...
| 34t | dep/no-dep/worktree-no-replace | `--dep --no-dep` matching → wt; go.mod unchanged |
| 34u | dep/no-dep/still-strict-not-a-dependency | `--dep --no-dep` not required → error; no external wt |
...
| 47f | all-deps/registered/no-dep | `--all-deps --no-dep` registered deps → external wts; no replaces; `wrked N` |
...
doctest vet ./tests/bring/no-dep ./tests/bring/verbose ./tests/dep/no-dep ./tests/all-deps/registered/no-dep
doctest test ./tests/bring/no-dep/...
doctest test ./tests/bring/verbose/...
doctest test ./tests/bring/help-mentions-no-dep
doctest test ./tests/dep/no-dep/...
doctest test ./tests/all-deps/registered/no-dep
