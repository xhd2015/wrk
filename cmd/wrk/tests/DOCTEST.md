...
- **wrk --scan-git-repos [ROOT...]** — standalone mode; mutually exclusive with other modes (`--projects`, `--add`, `--list`, create, etc.). Discovers git directories under each root via `scan_repo.Scan` (default product CacheRoot unless overridden), filters **`RepoTypeMain` only** (skips linked worktrees), and records each main with `storage.RecordProject(wrkHome, path, source)` where source is **`scan`** (`storage.SourceScan = "scan"`). **Roots**: remaining positionals after flags; when no roots are given, default root is **`$HOME` (`~`)** if it is a directory (not `~/Projects`); empty/unresolvable/non-directory home → clear error mentioning home or `~`. **Streaming / OnRepo**: for each newly discovered main, **record-as-found** then print immediately via `scan_repo.Options.OnRepo` — **discovery order** (CLI root order + walk order), **not** sorted after the full scan. **Stdout**: each **newly** recorded absolute main path on its own line as soon as it is found (trailing `\n` after last); already-known paths silent on stdout (optional summary on stderr). Exit 0 on success; partial root errors may warn on stderr. **Ctrl-C / SIGINT / SIGTERM**: cancel the scan context, keep partial `projects.json` (and scan cache already written), write stderr `warning: scan interrupted; progress saved (cache and newly recorded projects)` (must include `warning:` plus interrupt/progress-saved intent), and exit **130** via silent `ExitCodeError{Code: 130}` (no second hard-error body). **`--no-cache`**: bool flag valid **only** with `--scan-git-repos`; passes `NoCache: true` to `scan_repo.Scan` (no cache read/write); bare `wrk --no-cache` → non-zero, stderr `wrk: --no-cache is only valid with --scan-git-repos`. **Debug (P3)**: `scan_repo.Options.Debug = verbose || envTruthy(WRK_SCAN_DEBUG)` where truthy is `1`/`true`/`yes` (case-insensitive); pass `Options.Stderr = os.Stderr`. When Debug is on, stderr includes greppable phase-level `scan:` lines (`mode=warm|cold`, cache root, serve/refresh timing); when off, zero `scan:` markers from the scan path. Optional wrk-side summary `scan: record known=N newly=M`. Empty `CacheRoot` uses product default `$HOME/.cache/git-repo-scan`. Help documents `--scan-git-repos`, `--no-cache`, and default root `~`. Event `command: "scan-git-repos"`.
...
└── scan-git-repos/               # wrk --scan-git-repos discover + RecordProject source=scan
    ...
    ├── debug/                    # P3: wire Options.Debug from -v OR WRK_SCAN_DEBUG
...
# Run --scan-git-repos leaves (record + defaults + debug + streaming discovery-order + interrupt; skip labeled slow by default)
