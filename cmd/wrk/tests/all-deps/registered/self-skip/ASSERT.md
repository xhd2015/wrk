...
- Exit code 0.
- Stdout lists dep1 only, then `wrked 1 deps`.
- Consumer `go.mod` has a `replace` for `example.com/dep1` but NOT for `example.com/consumer`.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if allDepsHasReplaceForModule(mod, "example.com/consumer", "") {
		t.Fatalf("go.mod should NOT replace consumer's own module: %+v", mod.Replace)
	}
}
```