...
**Feature**: wrk --bring basename fallback to saved projects.json lookup

```
# --bring <basename> missing from cwd -> lookup projects.json by filepath.Base(path)
consumer (git + go.mod) -> wrk --bring <basename> -> stat(cwd/<basename>) fails -> match saved projects
...
- Consumer repo requires `example.com/dep` and is the process cwd for `wrk --bring`.
- Saved dep repos are registered via `wrk --add` and include a valid Go module.

## Steps

- Descendants configure consumer repo, saved dep paths, `--bring` basename argument, and optional `WRK_BASENAME_CONFIRM` + stdin for TTY selection.
- `req.Args = []string{"--bring", <basename>}` unless the scenario uses a path-with-separator argument.
...
func initSavedDepRepo(t *testing.T, workRoot, parent, basename string) string {
	...
	writeFile(t, filepath.Join(repoPath, "go.mod"), "module "+bringDepModulePath+"\n\ngo 1.22\n")
...
// initConsumerForBringBasename creates a consumer repo requiring example.com/dep.
func initConsumerForBringBasename(t *testing.T, workRoot string) string {
	t.Helper()
	return initBringConsumerRepo(t, workRoot, true)
}
...
func ensureBringBasenameFallbackHelpersUsed() {
	_ = resolvePath
	_ = initSavedDepRepo
	_ = recordSavedProject
	_ = initConsumerForBringBasename
