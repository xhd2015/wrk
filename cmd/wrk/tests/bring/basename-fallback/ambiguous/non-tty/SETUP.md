...
**Feature**: ambiguous --bring basename in non-TTY errors with candidate list

```
aaa/mydep + zzz/mydep saved
non-TTY wrk --bring mydep -> error listing both absolute paths; no external worktree
```
...
3. Run `wrk --bring mydep` without `WRK_BASENAME_CONFIRM`.
