...
- Non-zero exit code.
- Stderr reports `does not exist` for the cwd-resolved `sub/mydep` path.
- Stdout is empty.
- No external worktree created from the saved `mydep` project.
...
does not exist
	...
	candidate := resolvePath(t, filepath.Join(req.RepoDir, "sub", "mydep"))
