...
**Feature**: wrk --bring succeeds when the consumer repo has no root go.mod but a subdirectory has one

```
# consumer go.mod lives in go-pkgs/, no go.mod at root -> wrk --bring should scan
...
3. Run `wrk --bring <dep>` from the consumer repo root (not from go-pkgs/).
