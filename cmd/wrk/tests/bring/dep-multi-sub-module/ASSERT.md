...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if !bringHasReplaceForModule(mod, bringDepModulePathB, wantReplacePath) {
		t.Fatalf("go.mod missing replace %s => %s: %+v", bringDepModulePathB, wantReplacePath, mod.Replace)
	}
	// must NOT have a replace for the unmatched sub-module
	if bringHasReplaceForModule(mod, bringDepModulePathA, filepath.Join(wantPath, "a")) {
		t.Fatalf("go.mod should NOT have replace for %s: %+v", bringDepModulePathA, mod.Replace)
	}
