...
**Feature**: `Varargs` `--bring d1 d2` stops at `--no-dep` and skips replace/tidy

```
# Varargs consumes d1 d2 then continues; --no-dep is a flag, not a value
consumer requires dep1+dep2
  -> wrk --bring <d1> <d2> --no-dep
...
3. Run `wrk --bring <d1> <d2> --no-dep` (one `--bring`, two values, then flag).
...
func Setup(t *testing.T, d *session.Doctest, req *Request) error {
	...
	req.Args = []string{"--bring", dep1, dep2, "--no-dep"}
	return nil
}
```
