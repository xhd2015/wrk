...
- Exit code 0.
- Stdout is exactly two lines: external path for mydep1 then mydep2 (left→right), trailing `\n`.
- Both external paths exist as linked worktrees owned by their respective dep mains.
- Branches are `main-{WRK_DATE}` on each dep.
...
- Last `events.jsonl` event: `command=="bring"`; `args` include `--bring` and **both** dep paths.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	ev := multiLastEvent(t, req.WrkHome)
	if ev.Command != "bring" {
		t.Fatalf("event command: want %q, got %q args=%v", "bring", ev.Command, ev.Args)
	}
	if !multiEventArgsContain(ev.Args, "--bring") {
		t.Fatalf("event args should include --bring, got %v", ev.Args)
	}
	if !multiEventArgsContain(ev.Args, req.DepPath) {
		t.Fatalf("event args should include first dep path %q, got %v", req.DepPath, ev.Args)
	}
	if !multiEventArgsContain(ev.Args, req.SecondRepo) {
		t.Fatalf("event args should include second dep path %q, got %v", req.SecondRepo, ev.Args)
	}
}
```
