...
- Exit code 0.
- Stdout abs external path; replace present in go.mod.
- Stderr contains timestamped `$ go -C <consumerModDir> mod tidy` (or equivalent `-C` form).
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	assertBringStderrContainsTidyPreLine(t, resp.Stderr, req.ConsumerModDir)
}
```
