...
- Both externals exist under the new WT; replaces applied; source `src` untouched.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if createBringStdoutHasLine(resp.Stdout, ext1) || createBringStdoutHasLine(resp.Stdout, ext2) {
		t.Fatalf("stdout must not include external paths under --here; got %q", resp.Stdout)
	}
