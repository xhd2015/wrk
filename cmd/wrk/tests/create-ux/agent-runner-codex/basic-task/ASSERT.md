...
- Outer agent-run invoked with `--agent-runner=codex-tty`.
- Last argv element is exactly `$brainstorm fix bug` (not `/brainstorm fix bug`).
- No space; no iterm.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	args := assertAgentRunInvokedWith(t, req, wt, req.TaskDesc, "codex-tty", "$brainstorm")
