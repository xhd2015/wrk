...
**Feature**: wrk --dep replaces only in consumer modules that actually depend on the dep

```
# consumer has go-pkgs/go.mod (requires dep) and tools/go.mod (does NOT require dep) -> wrk --dep -> replace only in go-pkgs/
consumer (go-pkgs/ requires dep, tools/ does not) + dep -> wrk --dep -> replace in go-pkgs/go.mod only
```
