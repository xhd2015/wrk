...
**Feature**: wrk --dep replaces in all consumer modules that depend on the dep

```
# consumer has go-pkgs/go.mod and tools/go.mod, both requiring dep -> wrk --dep -> replace in both
consumer (go-pkgs/ requires dep, tools/ requires dep) + dep -> wrk --dep -> replace in go-pkgs/go.mod and tools/go.mod
```
