...
**Feature**: wrk --dep works when cwd is inside a sub-module directory (not the repo root)

```
# consumer cwd = consumer/go-pkgs/ (the dir with go.mod) -> wrk --dep -> success
consumer (cwd = go-pkgs/) + dep -> wrk --dep -> external worktree + replace
```
