...
**Feature**: wrk --dep from inside a linked consumer worktree registers the external worktree under the DEP repo (not the consumer's main repo)

```
# wrk <consumer-main>         -> consumer linked worktree at ~/.wrk/worktrees/consumer-...
# wrk --dep <dep> (from there) -> external/mydep-main-{date}
...
worktree, then `wrk --dep $X/dot-pkgs` run from inside it spawns the dep
