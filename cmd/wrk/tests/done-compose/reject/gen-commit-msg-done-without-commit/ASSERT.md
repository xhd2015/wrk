...
- Prefer require/required/must/need wording over bare mode mutex alone.
- Must not look like a successful primary path.

## Errors

- `--gen-commit-msg` with `--done` requires `--commit`.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if resp.ExitCode == 0 {
		t.Fatalf("expected non-zero exit for --gen-commit-msg --done without --commit, got 0 stdout=%q stderr=%q",
	...
	if !strings.Contains(lower, "require") &&
		...
		// Generic mutex without --commit-require intent is insufficient for this leaf.
