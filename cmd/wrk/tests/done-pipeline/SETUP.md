...
**Feature**: after successful `--done`, optional post-land ship via `runComposePostShip`:
apply runs enabled lanes concurrently; dry-run stays serial ordered.

```
# primary --done succeeds (not aborted) then compose ship + reinstall tail
linked wt (ahead) [+ optional wtB] [+ bare origin] [+ GOBIN stubs]
...
- Git available; monotree root helpers (`setupWrkWorktreeFromMain`, `setupCompositionTwoWTs`,
  `commitAheadOnWorktree`, `primaryThenSyncStdout`, `v2StdoutTemplate`, `assertShipStderrMarkers`, …).
