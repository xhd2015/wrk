...
- Stdout does **not** contain `synced:`, `tag created`, `tagged `, `pushed main`, or propagate vocabulary.
- Worktree still exists; main does not have `feature-work`; branch still present.
- Tag `v0.0.2` does **not** exist locally.
- Flag layer must accept `--propagate-tags` with `--done` (no mutual-exclusion error).
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if strings.Contains(resp.Stderr, "mutually exclusive") {
		t.Fatalf("flag layer still rejects done+propagate-tags on abort path; stderr=%q", resp.Stderr)
	}
