...
**Feature**: aborted `--done` runs no sync / tag-next / push even when all flags present

```
# user declines confirm → merge-back aborted; no post-pipeline
myrepo (v0.0.1) + wtA (ahead)
  -> wrk --done --confirm-from-stdin --sync --tag-next --push  (stdin: n)
  -> merge-back aborted
...
func Setup(t *testing.T, req *Request) error {
	...
	req.Args = []string{"--done", "--confirm-from-stdin", "--sync", "--tag-next", "--push"}
	req.StdinInput = "n\n"
	return nil
}
```
