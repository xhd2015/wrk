...
- Exit code 0 (absolute intra-repo replace is tolerated under the default lenient guard).
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if resp.ExitCode != 0 {
		t.Fatalf("expected exit 0 (absolute intra-repo warn + proceed), got %d stdout=%q stderr=%q", resp.ExitCode, resp.Stdout, resp.Stderr)
	}
