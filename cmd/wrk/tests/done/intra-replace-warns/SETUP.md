...
**Feature**: wrk --done silently proceeds for a relative intra-repo filesystem replace

```
# consumer go.mod has replace example.com/foo => ./submod; ./submod is a real
# nested module sharing the consumer's git toplevel -> relative intra-repo ->
# silent proceed
consumer wt (replace => ./submod, same repo) -> wrk --done -> no warn, merge-back runs
```
...
1. Create main repo on `main` with a consumer `go.mod` whose
   ...
   stands in for the user's `replace … => ../../pkgs`: both resolve to an
   existing directory sharing the consumer's `git rev-parse --show-toplevel`,
   i.e. relative intra-repo.
...
shares the consumer's toplevel). Under the default lenient guard a **relative**
intra-repo replace is **silent** (no stderr warn) and `--done` proceeds (exit 0),
so merge-back runs and removes the worktree.
...
func Setup(t *testing.T, d *session.Doctest, req *Request) error {
	...
	// Consumer module with a relative intra-repo filesystem replace.
