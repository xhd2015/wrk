...
**Feature**: wrk --dep when both consumer and dep go.mod live in subdirectories

```
# consumer go.mod in go-pkgs/ requires example.com/dep/sub; dep go.mod in sub/ -> wrk --dep -> replace in go-pkgs/go.mod => <external>/sub
consumer (go-pkgs/ requires dep/sub) + dep (sub/ has go.mod) -> wrk --dep -> external worktree + replace
```
