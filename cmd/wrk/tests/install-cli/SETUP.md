...
**Feature**: `wrk --install name...` resolves named local binaries like
`--reinstall-local` but installs them without a GOBIN/GOPATH/bin presence gate

```
# module fixture + isolated GOBIN -> wrk --install <name...> [--dry-run] [--main]
# dry-run: would: go install|go run lines + would: install N binaries; zero mutation
# execute: go install|go run progress + installed N, failed F (soft when failed > 0)
WorkRoot/{mod,gobin,.wrk}
  -> wrk --install <name...> [--dry-run] [--main]   (cwd=mod, GOBIN=gobin)
  -> exit 0 on plan/execute (incl. soft failures + warning:) | non-zero on hard errors
```

## Preconditions

- Nested root: no inheritance from `cmd/wrk/tests` (own `DOCTEST.md`).
- Go toolchain on PATH; L3 leaf builds the session `wrk` binary once to
  `{DOCTEST_FIXTURE_ROOT}/{DOCTEST_SESSION_ID}/bin/wrk`.
- Fixture modules are real directories under a per-leaf `t.TempDir()` WorkRoot:
  `ModuleRoot` is process cwd (module root, multi tree, or git worktree);
  `BinDir` (`gobin`) holds stub files named after bins (or real go-installed
  binaries after execute).
- `--install` never requires the bin to exist in `BinDir`: leaves assert the
  absence of `skip:` lines, not the presence of stubs.

## Steps

1. Root `Setup` creates isolated `WorkRoot`, `WrkHome`, empty `ModuleRoot`,
   empty `BinDir`.
2. Leaves write go.mod / package mains / stubs as needed, then set `Args`.
3. Root `Run` uses `wrkcli.Capture` (L2, `req.InProcess = true`) unless the
   leaf needs the product-binary boundary (L3, `execute/installs-bin`).
4. GOBIN is always forced to `req.BinDir` so no leaf can touch a developer's
   real bin directory.

## Context

- `--install` shares the `--reinstall-local` discovery engine (cmd/<name> →
  `go install`, script/<name>/install → `go run`, nearest-go.mod re-root) and
  differs in gate (always forced) and vocabulary (`would: install N binaries`,
  `installed N, failed F`).
- Last content line of successful dry-run / execute stdout is the summary line.
- Errors are hard (non-zero) for unknown, ambiguous, or multi-module-claimed
  names; install *failures* are soft (exit 0 + stderr `warning:`).
- Exact stdout uses `assert.Output` v2 templates where bounded; help and error
  leaves use substring checks.
...
func Setup(t *testing.T, d *session.Doctest, req *Request) error {
	...
	ensureInstallCLIHelpersUsed()
	return nil
}

// installCLIEnv isolates WRK_HOME and forces GOBIN to the leaf bin dir.
// Strips ambient NO_COLOR so leaves own color policy (pipe = plain; --color on).
// ExtraEnv is appended last so leaf overrides can win on duplicate keys.
func installCLIEnv(req *Request) []string {
...
// assertBinNotStub checks execute replaced the stub with a real install.
...
// assertInstallSummary checks the last non-empty stdout line is the
// --install execute summary "installed N, failed F".
func assertInstallSummary(t *testing.T, stdout string, installed, failed int) {
	t.Helper()
	want := fmt.Sprintf("installed %d, failed %d", installed, failed)
	...
	if last != want {
		t.Fatalf("install summary %q want %q\nfull stdout:\n%s", last, want, stdout)
	}
...
// assertBinNotExists checks GOBIN/<name> was not created (dry-run / zero mutation).
func assertBinNotExists(t *testing.T, binDir, name string) {
	t.Helper()
	path := filepath.Join(binDir, name)
	if _, err := os.Stat(path); err == nil {
		t.Fatalf("expected %s to be absent after dry-run", path)
	}
}
...
// --- events.jsonl helpers (command "install") ---
...
// assertLastEventCommandInstall checks the last events.jsonl entry is
// command "install" with the given exit code and required CLI args.
func assertLastEventCommandInstall(t *testing.T, wrkHome string, wantExit int, requiredArgs []string) {
	t.Helper()
	...
	if ev.Command != "install" {
		t.Fatalf("event command: want %q, got %q (all=%v)", "install", ev.Command, events)
	}
...
func ensureInstallCLIHelpersUsed() {
	_ = writeGoMod
	_ = writePackageMain
	...
	_ = assertBinNotExists
	_ = assertBinNotStub
	_ = assertBinExecutable
	_ = assertBinRuns
	_ = assertInstallSummary
	...
	_ = v2StdoutTemplate
	_ = readEvents
	_ = assertLastEventCommandInstall
	_ = eventArgsContain
	_ = resolvePath
	_ = eventsJSONLPath
	_ = filterEnvKeys
