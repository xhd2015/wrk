...
**Feature**: after successful `--merge-back`, optional post-land ship via `runComposePostShip`
(source worktree kept): apply concurrent; dry-run serial ordered.

```
# primary --merge-back succeeds then compose ship; source wt kept
linked wt (ahead) [+ optional wtB] [+ bare origin]
  -> wrk --merge-back -y [--sync] [--tag-next] [--push]
  -> merge-back (message on stdout; worktree kept)
  ...
  -> dry-run: serial would: sync → tag-next → push (blank lines; stdout largely unchanged)
  -> event command stays "merge-back"
```

## Preconditions

- Git available; monotree root helpers (`assertShipStderrMarkers`, …).
- **Real apply** post-pipeline after merge-back (composition dry-run under `dry-run/`; done twin under `done-pipeline/`).
- Locked behavior:
  1. dispatch prefers **merge-back** over bare `runTagNext` when primary set,
