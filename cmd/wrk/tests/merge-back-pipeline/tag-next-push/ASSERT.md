...
- Exit code 0.
- Single-lane live stdout: primary, blank line, then tag-next apply immediately followed by push confirm (one ship lane; no blank between tag and push).
...
- Branch tip and new tag published to origin via tag-next+push lane.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	// Single tag-next+push lane: tag apply + push confirm with no blank between.
	want := composeLandShipStdout(joinMajorStages(
		primaryMergeMsg(req.WtBranch),
		tagNextRootBumpApplyStdout(short)+mergeBackPushConfirmLine(),
	))
