...
- Prefer mentioning `--pin-locals` and/or `--add-all`.
- Stdout empty (or no successful pin-local plan).

## Errors

- `--pin-locals` cannot be combined with `--add-all`.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if !strings.Contains(se, "--pin-locals") &&
		...
		!strings.Contains(se, "--add-all") {
		t.Fatalf("stderr should name --pin-locals and/or --add-all, got %q", se)
	}
}
```
