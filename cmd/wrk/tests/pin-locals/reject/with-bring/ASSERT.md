...
- Prefer mentioning `--pin-locals` and/or `--bring`.
- Stdout empty (or no successful pin-local plan).

## Errors

- `--pin-locals` cannot be combined with `--bring`.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if !strings.Contains(se, "--pin-locals") &&
		...
		!strings.Contains(se, "--bring") {
		t.Fatalf("stderr should name --pin-locals and/or --bring, got %q", se)
	}
}
```
