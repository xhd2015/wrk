...
- Prefer mentioning `--pin-locals` and/or `--unwind`.
- Stdout empty (or no successful pin-local plan).

## Errors

- `--pin-locals` cannot be combined with `--unwind`.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if !strings.Contains(se, "--pin-locals") &&
		...
		!strings.Contains(se, "--unwind") {
		t.Fatalf("stderr should name --pin-locals and/or --unwind, got %q", se)
	}
}
```
