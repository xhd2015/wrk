...
- Prefer mentioning both `--pr` and `--done` when product wording allows.

## Errors

- `--pr` and `--done` cannot be combined (P1; compose pipelines are P2).
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if resp.ExitCode == 0 {
		t.Fatalf("expected non-zero for --pr --done, got 0 stdout=%q stderr=%q",
