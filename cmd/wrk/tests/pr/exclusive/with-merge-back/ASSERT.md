...
- Prefer mentioning both `--pr` and `--merge-back` when product wording allows.

## Errors

- `--pr` and `--merge-back` cannot be combined.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if resp.ExitCode == 0 {
		t.Fatalf("expected non-zero for --pr --merge-back, got 0 stdout=%q stderr=%q",
