...
- No successful PR stdout tokens (`PR created` / `comment added`).
- Fake `gh` must **not** call `pr create` or `pr comment`.

## Errors

- Main-repo cwd is refused for `--pr` (linked worktree only).
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if resp.ExitCode == 0 {
		t.Fatalf("expected non-zero exit for --pr on main repo; stdout=%q stderr=%q",
