...
- Non-zero exit code.
- Stderr reports `does not exist` for the cwd-resolved `sub/foo` path.
- Stdout is empty.
- No worktree created from the saved `foo` project.
...
does not exist
	...
	candidate := resolvePath(t, filepath.Join(req.RepoDir, "sub", "foo"))
