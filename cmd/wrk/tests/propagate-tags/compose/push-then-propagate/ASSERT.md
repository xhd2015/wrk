...
Same two major stdout stages as tag-then-propagate (tag-next does not print a
separate push confirmation line for tag-only push — same as bare
`wrk --tag-next --push`):
...
- Bare origin has `refs/tags/v1.0.1`.
- Consumer require bumped and committed (same as tag-then-propagate).

## Side Effects

- Source HEAD unchanged; new local + remote tag.
