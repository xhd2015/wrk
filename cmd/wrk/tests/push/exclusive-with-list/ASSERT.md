...
- Prefer mentioning both `--push` and `--list` when product wording allows.

## Errors

- `--push` and `--list` cannot be combined.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if resp.ExitCode == 0 {
		t.Fatalf("expected non-zero exit for --push --list, got 0 stdout=%q stderr=%q",
