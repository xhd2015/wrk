...
│   ├── documents-new-flags/      # SKILL.md names --propagate-tags, --projects-dep-graph, --pr/--title/--comment
...
| A3b | show/documents-new-flags | `wrk skill --show` → documents `--propagate-tags`, `--projects-dep-graph`, and `--pr`/`--title`/`--comment` |
