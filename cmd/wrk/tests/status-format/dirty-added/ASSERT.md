...
- Status is dirty with one **added** from the untracked file (`??` → wrk `added`).
- Zero changed, renamed, and deleted; all four buckets appear in the dirty string.
- Stderr is empty.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	assert.Output(t, resp.Stdout, statusFormatBlockTemplate(t, req.MainRepo,
		"dirty (1 added, 0 changed, 0 renamed, 0 deleted)"))
}
```
