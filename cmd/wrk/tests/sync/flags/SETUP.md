...
- `--dry-run` is valid with `--sync` (and existing modes such as `--tag-next`).
- Bare `wrk --dry-run` remains rejected; host list includes `--done`, `--merge-back`, `--tag-next`, and `--sync` (plus other hosts such as `--push` / `--reinstall-local`).
