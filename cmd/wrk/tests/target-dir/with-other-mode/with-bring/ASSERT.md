...
- Non-zero exit code.
- Stderr contains `wrk: unexpected arguments` (target-dir is create-only; `--bring` rejects it).
- Stdout is empty.
...
- `<target-dir>` combined with `--bring`.
...
> implementation already rejects any extra positional alongside `--bring` with
