...
**Feature**: <target-dir> + --dep <dep> → wrk: unexpected arguments (target-dir is create-only)

```
# consumer requires dep; --dep is not the create path; second positional rejected
consumer (go.mod + git) + dep repo -> wrk <consumer> <target-dir> --dep <dep> -> non-zero
```
...
5. Set `req.Args = ["--dep", {WorkRoot}/mydep]`.
6. Run `wrk consumer {WorkRoot}/wt --dep {WorkRoot}/mydep` from process cwd `{WorkRoot}`.
