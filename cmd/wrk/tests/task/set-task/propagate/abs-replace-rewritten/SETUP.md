...
```
# wrk --bring writes replace example.com/dep => <abs>/external/mydep-...
# --set-task moves consumer wt → go.mod replace must point at new external path
consumer wt + wrk --bring (abs replace) -> wrk --set-task -> go.mod replace updated
```

## Steps

1. Create consumer main + dep, spawn consumer wt with `--task "old slug"`.
2. Run `wrk --bring` from consumer wt (writes absolute replace in go.mod).
...
req.InProcess = true
	...
	// Precondition: wrk --bring wrote an absolute replace under the old consumer path.
	...
	if !filepath.IsAbs(oldReplace) {
		t.Fatalf("wrk --bring should write absolute replace, got %q", oldReplace)
	}
