...
**Feature**: wrk --task "fix login bug" produces a worktree with slug in dir and branch names

```
wrk -> [--task "fix login bug"] -> git worktree add -> path on stdout
```
...
2. Run `wrk --task "fix login bug"` (handled by Run via req.TaskDesc).
