...
- Dir and branch names include `-fix-login-bug` after the date.
- Last `events.jsonl` event has `command: "create"`, `exit_code: 0`, `args: ["-t", "fix login bug"]`.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	assertLastEvent(t, req.WrkHome, "create", 0, req.MainRepo, req.MainRepo, []string{"-t", "fix login bug"})
}
```