...
dirty pending free-first
  ...
  -> after peeling dep U: Pin consumers + go mod tidy
  -> --done removes linked WT; reinstall soft on fail
```

## Preconditions

- Parent helpers: `setupApplyLeafPinStack`, `setupApplyAlreadyMainRootBump`,
  pin/tag/origin asserts, local bare remotes, optional `file://` modproxy.
- Leaves set `req.InProcess = true` and full `req.Args` **without** `--dry-run`.
- Classic TDD: product may still return apply-not-implemented → **RED** until
  implementer lands peel/pin apply (P3 dry-run contracts must stay GREEN).
