...
- Embedded version lives in `wrkcli/VERSION.txt` (compiled into the package under test).
...
- Only `WRK_HOME` is passed via Capture env; no other wrk env overrides.
...
func versionCaptureEnv(req *Request) []string {
