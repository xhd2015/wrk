...
- Non-zero exit.
- Stdout empty.
- Stderr requires full GitHub pull request URL (reject `owner/repo#N`).

## Errors

- Shorthand `owner/repo#N` is not accepted.
...
func Assert(t *testing.T, d *session.Doctest, req *Request, resp *Response, err error) {
	...
	if resp.ExitCode == 0 {
		t.Fatalf("owner/repo#N must fail; stdout=%q stderr=%q", resp.Stdout, resp.Stderr)
	}
	if resp.Stdout != "" {
