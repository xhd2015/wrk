...
- Exit code 0 (`-y` synonym of cascade auto-yes on non-TTY).
- Dep fix merged into dep main.
- External and consumer worktrees removed.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if resp.ExitCode != 0 {
		t.Fatalf("expected exit 0 (cascade auto-yes with -y), got %d stdout=%q stderr=%q", resp.ExitCode, resp.Stdout, resp.Stderr)
	}
