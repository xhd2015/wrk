...
func isCreateMode(projects, projectsDepGraph, addFlagSet, removeFlagSet, setTaskFlagSet, whereFlagSet, repos, status bool, bring bool, reinstallLocal, tagNext, propagateTags, syncFlag, pushFlag, prFlag, list, done, mergeBack, cd, mainFlag, unwind bool) bool {
	if projects || projectsDepGraph || addFlagSet || removeFlagSet || setTaskFlagSet || whereFlagSet || repos || status || cd || mainFlag || unwind {
		return false
	}
	if bring || reinstallLocal || tagNext || propagateTags || syncFlag || pushFlag || prFlag || list || done || mergeBack {
		return false
	}
	return true
}
